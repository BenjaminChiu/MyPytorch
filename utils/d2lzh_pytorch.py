...
# ########################## 5.6 #########################3
def load_data_fashion_mnist(batch_size, resize=None, root='D:\DeepLearning_Data'):
    """Download the fashion mnist dataset and then load into memory."""
    trans = []
    if resize:
...
# 函数_make_list目的：将obj转为list或tuple；原本是下面的show_bboxes的内置函数
...
def assign_anchor(bb, anchor, jaccard_threshold=0.5):
    ...
    return torch.tensor(assigned_idx, dtype=torch.long).to("cuda:0")
...
def MultiBoxTarget(anchor, label):
    ...
    Returns:
        ...
        bbox_offset: 每个锚框的标注偏移量，形状为(bn，锚框总数*4) bn为batich size
    ...
    def MultiBoxTarget_one(anc, lab, eps=1e-6):
        ...
        assigned_bb = assigned_bb.to("cuda:0")
